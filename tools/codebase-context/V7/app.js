...
/* ---------- 过滤规则（data/filters.json 纯配置驱动；以下为内置兜底默认） ---------- */
let maxFileSize   = 5 * 1024 * 1024;     // 单文件上限（可被 filters.json maxFileSizeMB 覆盖）
let ignoredDirSet = new Set([
...
/* 加载过滤配置（根相对路径，勿用 ../——GitHub Pages 子路径部署会 404）；
   失败或字段缺失时保留内置默认值 */
function loadFilters() {
  ...
  filtersPromise = (async () => {
    ...
    if (cfg) {
      if (Array.isArray(cfg.ignoredDirs) && cfg.ignoredDirs.length) ignoredDirSet = new Set(cfg.ignoredDirs);
      if (Array.isArray(cfg.ignoredNames)) ignoredNameSet = new Set(cfg.ignoredNames);
      if (Array.isArray(cfg.textExts) && cfg.textExts.length) textExtSet = new Set(cfg.textExts);
      if (typeof cfg.maxFileSizeMB === "number" && cfg.maxFileSizeMB > 0) maxFileSize = cfg.maxFileSizeMB * 1024 * 1024;
    }
  })();
...
const DEFAULT_PROMPT_INTRO = "⚙️ 系统指令（System Prompt）：请严格遵循以下场景设定进行回答。";
let promptIntro = DEFAULT_PROMPT_INTRO;   // 全局引导文案（scenarios.json 顶层 promptIntro 覆盖）
let scenarios = [];
...
/* ---------- 目录树 ↔ 预览联动定位状态 ---------- */
let locatePath = null;                 // 当前定位的文件路径（null = 默认首尾折叠）
let dirFirstFile = new Map();          // 目录路径 → 该目录下第一个文件路径
...
function initLineNumbers() {
  try { els.lineNumToggle.checked = localStorage.getItem("cca-lineNumbers") === "1"; } catch (_) {}
}
...
function pathHasIgnoredDir(path) {
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    if (ignoredDirSet.has(parts[i])) return true;
  }
  return false;
}
function shouldInclude(path, name) {
  const base = name.toLowerCase();
  if (ignoredNameSet.has(base)) return false;
...
function treeDOM(node, prefix) {
  ...
  for (const [name, child] of node.children) {
    ...
    if (child.isFile) {
      ...
      row.addEventListener("click", () => locateFile(rel));
      li.appendChild(row);
    } else {
      ...
      row.prepend(tw);
      const nested = treeDOM(child, rel);
      ...
      row.addEventListener("click", () => {
        ...
        tw.textContent = open ? "▾" : "▸";
        const first = dirFirstFile.get(rel);
...
function getPromptIntro(scenario) {
  if (scenario && scenario.promptIntro) return scenario.promptIntro;
  return promptIntro || DEFAULT_PROMPT_INTRO;
}
...
async function renderResults(rootName, files, warnings) {
  ...
  updateExportData();
  dirFirstFile = buildDirFirstFile(files);
  locatePath = null;           // 重新扫描后重置定位状态，回到默认首尾折叠
...
function computeFileAnchors(md) {
  const lines = md.split("\n");
  let headerEnd = 0;
  let i = 0;
...
function buildDirFirstFile(files) {
  const m = new Map();
  for (const f of files) {
...
function renderLocatedPreview(lineNumbers) {
  const { headerEnd, files } = computeFileAnchors(fullMarkdown);
  const idx = files.findIndex((f) => f.path === locatePath);
  if (idx < 0) { locatePath = null; renderPreview(); return; }
  ...
  const bar = '<div class="locate-bar"><span>📍 已定位到 <code>' + escapeHTML(locatePath) + '</code></span>' +
  ...
  if (clearBtn) clearBtn.addEventListener("click", () => { locatePath = null; renderPreview(); });

  requestAnimationFrame(() => {
    const hs = els.preview.querySelectorAll("h3[data-path]");
    for (const h of hs) {
      if (h.getAttribute("data-path") === locatePath) { h.scrollIntoView({ block: "start" }); break; }
    }
  });
}
...
function renderPreview() {
  ...
  if (locatePath) { renderLocatedPreview(lineNumbers); return; }
...
async function processFiles(rootName, rawEntries) {
  ...
  await loadFilters();   // 确保过滤规则就绪（filters.json 或内置默认）后再过滤
  ...
  if (!scenariosPromise) {
    scenariosPromise = (async () => {
      try {
        ...
        scenarios = list;
        if (data && typeof data.promptIntro === "string" && data.promptIntro) promptIntro = data.promptIntro;
