...
(function () {
  ...
  function injectManageUI() {
    if (document.getElementById('v4-manage-section')) return;
    var target = document.getElementById('view-bookmarks');
    if (!target) return;

    var section = document.createElement('section');
    section.id = 'v4-manage-section';
    section.className = 'v2-manage-section';
    section.innerHTML =
      '<div class="v2-manage-header">' +
        '<span class="v2-manage-title">📂 管理书签</span>' +
        '<button class="v2-manage-btn-add" id="v4-btn-add">+ 添加</button>' +
      '</div>' +
      '<ul class="v2-manage-list" id="v4-manage-list"></ul>';
    target.appendChild(section);

    document.getElementById('v4-btn-add').addEventListener('click', function () {
      showModal();
    });
  ...
  function refreshManageList(bookmarks) {
    var list = document.getElementById('v4-manage-list');
    if (!list) return;
  ...
  /* 添加书签模态框 */
  function showModal() {
    var old = document.getElementById('v4-modal-overlay');
    if (old) old.remove();

    var overlay = document.createElement('div');
    overlay.id = 'v4-modal-overlay';
    overlay.className = 'v2-modal-overlay';
    overlay.innerHTML =
      '<div class="v2-modal">' +
        '<h3 class="v2-modal-title">添加书签</h3>' +
        '<label class="v2-modal-label">名称</label>' +
        '<input class="v2-modal-input" id="v4-modal-name" placeholder="书签名称" maxlength="60" />' +
        '<label class="v2-modal-label">链接</label>' +
        '<input class="v2-modal-input" id="v4-modal-url" placeholder="https://..." maxlength="500" />' +
        '<label class="v2-modal-label">图标 Emoji</label>' +
        '<input class="v2-modal-input" id="v4-modal-icon" placeholder="🚀" maxlength="4" />' +
        '<div class="v2-modal-actions">' +
          '<button class="v2-modal-btn v2-modal-btn-cancel" id="v4-modal-cancel">取消</button>' +
          '<button class="v2-modal-btn v2-modal-btn-confirm" id="v4-modal-confirm">确认</button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(overlay);

    overlay.addEventListener('click', function (e) { if (e.target === overlay) overlay.remove(); });
    document.getElementById('v4-modal-cancel').addEventListener('click', function () { overlay.remove(); });
    document.getElementById('v4-modal-confirm').addEventListener('click', function () {
      var nm = (document.getElementById('v4-modal-name').value || '').trim();
      var url = (document.getElementById('v4-modal-url').value || '').trim();
      var icon = (document.getElementById('v4-modal-icon').value || '').trim() || '🔗';
      if (!nm) { document.getElementById('v4-modal-name').focus(); return; }
      try {
        var raw = localStorage.getItem(STORAGE_KEY);
        var list = raw ? JSON.parse(raw) : [];
    ...
    setTimeout(function () {
      var inp = document.getElementById('v4-modal-name');
      if (inp) inp.focus();
    }, 100);
  }
