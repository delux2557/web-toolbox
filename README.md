...
| 🗃️ [Excel / CSV 转 SQL](./tools/excel2sql/) | ✅ 可用 | 把表格每一行转成可粘贴的 SQL：**UNION ALL 内联表**（CTE 体内缩进一级） / **INSERT 分批** × **5 种方言**（SQL Server / MySQL / Oracle / PostgreSQL / SQLite，与命令行版 0.4.1 对齐）；**表头行自动识别且可点选改判**（识别理由如实回显，宁可保守也不静默丢数据）；**列级类型统一**（同列混有文本则整列字符串化，避免隐式转换报错）；零依赖原生解析 `.xlsx`（ZIP + `DecompressionStream('deflate-raw')`，无需任何第三方库）/ `.csv`（UTF-8↔GB18030 编码探测 + 分隔符探测）；**CSV 默认还原数字列**（对齐命令行版 `infer_types = auto`：只在「需要且安全」时推断，Excel 源不动）、**前导零与超过 15 位有效数字的列保持文本**（JS 双精度会悄悄改写末尾几位，且 19 位以上超 `bigint`）、**纯日期默认按命令行版行为输出 `00:00:00`** 并提供精确判定开关；每份产物带头注释说明「哪些列是被迫按字符串输出的、为什么」；解析、生成全在本地，**文件不上传**。界面侧：**右侧「数据预览 / SQL 产物」页签分时复用整块高度**（含疑问圆点提示、记住上次选择）；**改选项即自动重算**（350ms 防抖、无需点按钮），**超过 5000 行降级为手动并说明原因**；`Alt`+`Z` 自动换行（消掉横向滚动条）；方言/输出形式/包裹方式收成下拉（选项从单一来源派生）；贴底状态栏给出行列数、SQL 行数、体积与耗时。零依赖多文件静态页，**支持 `file://` 双击直接用** |
...
- **第 1 层（139 项）** 把 `js/` 下四个模块按页面加载顺序拼进一个函数体直接跑，覆盖
  零依赖解析真 `.xlsx`（两个夹具：openpyxl 的 `inlineStr` 与真实 Excel 的 `sharedStrings`）、
  表头打分与两道保护、列级类型统一、五方言的字面量与转义（含 `']'`、`'` 的注入防护；
  SQLite 的 `||` / `CHAR(10)` / 无 `TO_DATE`）、**CTE 体内缩进**、
  **CSV 数字推断的两条安全边界**（有效数字 > 15 位、转 float 丢尾数的一律留文本）、
  **`all-string` 与推断的联动**、方言解析**不静默回退**（未知值必须抛 `UnknownDialect`）、
  方言清单**单一来源**（`ORDER` 与注册表/下拉/编号三者一致）、
  日期兼容模式开关、性能护栏阈值；另含一批**静态一致性**断言（`app.js` 引用的 id 都在 `index.html`、
  脚本加载顺序、`settings` 里的 SQL 参数都真接进了 `sqlOptions()`、纯界面状态**没有**混进
  `sqlOptions()`）。
- **第 2 层（281 项）** 用一个**记账式 DOM 桩**加载全部 7 个脚本，然后按真实用户路径走一遍：
  拖入文件 → 表头识别与点选改判 → 换方言 → 切包裹方式 → 开关日期模式 → 生成 → 下载 → 退回空态；
  另有一节专测**页签 / 自动换行 / 「⋯」菜单 / 防抖自动生成**，以及**分级策略**（拖 6001 行夹具
  进去，验证它确实降级为手动、且点「立即重新生成」照常能出结果）。
  三条桩上的纪律值得记下来：① `getElementById` 的注册表**从 `index.html` 预填**（含 `hidden` 属性
  **与真实 tagName**），查不到就返回 `null` —— 于是"id 拼错"变成一条免费断言；
  ② `textContent` 按真实 DOM 语义建模（节点序列拼接，`appendChild` 不顶掉已有文本），
  否则"文本有没有写进去"会假阴；③ `execCommand` 留**可控的失败开关**，把"复制两条路都不通"
  那条降级分支真跑到。另有一节**静态契约**直接读源文件：桩不解析 HTML 文本，所以排版类前提
  （`[hidden]` 的 `!important`、分栏由 grid 表达、右列不再是"按 max-content 定高的自动轨道 +
  百分比限高"那套会留死区的组合、次级文字对比度门槛、窄屏堆叠的 flex 前提）只能在这一层守。
  ⚠️ 读源文件做正则断言前**必须先剥掉 CSS 注释** —— 注释里为说明历史决策而引用的旧规则原文
  会被 `\.esq-right\s*\{([^}]*)\}` 先命中，拿到的是注释里的示例代码而不是真实规则。

两层都做过**反向实验**（故意改坏源文件，逐条确认"该红的那条真红了、且还原来即绿"）。
这一步抓出过**五条测试自身的问题**：一条**假守卫**（断言"有动作按钮"却命中了没有 click 处理器的包裹层
`<div>`）、一处**覆盖盲区**（「裸 SELECT」包裹方式从没被点开过）、以及一条**选错载体的断言**
（"含第 2 行"在"保守取第 1 行"的兜底文案里也成立）；改版后又抓到两条 ——
**拿默认值当期望值**（页签持久化断言读 `tab === 'result'`，而 `result` 正是默认值，
别处的 `saveSettings()` 早把它写进去了，于是"存了没有"根本区分不出来）、
**拿不可区分的观测量做断言**（防抖断言在同步栈里读"产物还没更新"，而 `generate()` 是 async、
第一句就 `await`，它本来就还没渲染 —— 把防抖整个拿掉也照样绿）。
顺带发现产品侧四个真问题并修掉：`innerHTML` 拼字符串（改为纯 DOM 节点）、
分段控件每点一次就重建按钮（按钮被换成新对象、丢掉键盘焦点）、
**导入路径绕过分级阈值**（`applyBook` 直接 `generate(true)`，6001 行文件照样自动吐 12,009 行 SQL）、
以及**状态栏永久停在「处理中…」**（`setBusy(false)` 不回收自己写下的状态行）。
...
后者保证不只是"变红了"，还"红对了地方"。当前 17 条用例全部咬到。

**不接进 CI**，因为 ① 它会改源文件；② 每条用例都要起一次子进程跑整道闸门，偏慢（现在约 2 分钟）。
改完断言或布局后手动跑一次即可。
...
- **变异必须自证真的动到了文件**。`find` 命中 0 处或 >1 处一律判为"用例自身无效"并报错退出 ——
  ...
  （注意本仓库行尾并不统一：`styles.css` 是 CRLF，新写的 `js/*.js` 是 LF）。
- **`expect` 要写具体的断言文案**，别只判"有没有失败"。同一次改坏常常连带多条断言变红，
  只判"红了"会掩盖"该红的那条其实没红"。
- ★★ **还原必须扛得住强杀**。还原原先只写在 `finally` 里，而进程被 SIGTERM 打断
  （用例变多、变慢之后这很常见）时 `finally` 根本不执行 —— 源文件就**静默留在改坏状态**，
  且事后没有任何提示：下一次运行只会打印「基线就不绿」，极易被误读成闸门坏了。
  真踩到：`app.js` 里留着一句 `else if (false)`，我还据此把一次真机观察误判成了"修复没生效"。
  现在改成**动手前先把原文写进 `tools/_build/.mutation-journal.json`**（已 gitignore），
  正常还原后复核通过才撤掉；启动时若发现 journal 还在，先用它还原再继续。
  信号处理（SIGINT/SIGTERM/SIGHUP）只是"更早一点"，journal 才是兜底（SIGKILL 拦不住）。
  这条机制本身有确定性测试：`node tools/_build/selftest-journal.mjs`（手工构造中断现场，5 项断言）。
...
改版时又踩到同一族问题的**另一个变体**：为说明历史决策，我在 CSS 里留了一段注释，里面写着
旧的 `.esq-right { grid-template-rows: … }` 原文 —— 于是「右列不再用那套会留死区的组合」
这条断言读到的**是注释里的示例代码**，真实规则怎么改它都是红的。同一族的第三种变体是
**子串匹配**：`has(css, '.esq-row')` 会被 `.esq-rownum`（产物行号列）满足，
那个选择器整个删掉也照样通过。现在这一节的断言一律 ① 先剥注释、② 用 `选择器\s*\{` 定位真规则。
...
| `check-excel2sql.mjs` | 139 · 解析 / 表头 / 类型 / 五方言渲染 / 推断安全边界 / 静态一致性 |
| `check-excel2sql-ui.mjs` | 281 · DOM 桩下的界面接线与真实操作序列 + 静态契约（布局/对比度门槛） |
